...
if [ $CHANGES -eq 1 ] || $FORCE_DEPLOY; then
    ...
    if [[ -d "../lamasiaweb" ]]; then
        rm -r ../lamasiaweb/*
    fi

    cp -r ./backend/* ../lamasiaweb/

    if [[ -d "../lamasiaweb/node_modules" ]]; then
        rm -r ../lamasiaweb/node_modules
    fi

    if [[ -f "../lamasiaweb/package-lock.json" ]]; then
        rm ../lamasiaweb/package-lock.json
    fi

    cd ../lamasiaweb && git add .
